...
extern "C" fn vita49_telemetry_callback(
    ...
    if !stats_json.is_null() {
        let payload = unsafe { CStr::from_ptr(stats_json) }.to_string_lossy().into_owned();
        let _ = app_handle.emit("vita49-stream-stats", payload);
    }

    if !packet_batch_json.is_null() {
        let payload = unsafe { CStr::from_ptr(packet_batch_json) }.to_string_lossy().into_owned();
        let _ = app_handle.emit("vita49-packet-batch", payload);
    }
}
...
impl FersContext {
    ...
    fn run_simulation_ex(
        ...
        let telemetry_callback: ffi::fers_vita49_telemetry_callback_t =
            if emit_vita49_telemetry { Some(vita49_telemetry_callback) } else { None };
